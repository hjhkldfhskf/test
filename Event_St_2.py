...
# 发布者密码（请务必修改！）
PUBLISHER_PASSWORD = "admin123"  # ← 建议修改为更安全的密码
...
def main():
    ...
    if st.session_state.get("publisher_logged_in", False):
        ...
        if "device_id" in all_scores.columns:
            valid_submissions = all_scores[all_scores["device_id"].notna() & (all_scores["device_id"] != "")]
            submitted_count = valid_submissions["device_id"].nunique()
        else:
            submitted_count = 0
...
def display_final_scores_publisher():
    ...
    if valid_scores.empty or valid_scores["总分"].sum() == 0:
        st.sidebar.info("📭 暂无评分数据")
        return

    # 计算平均分
    final_scores = valid_scores.groupby(["编号", "姓名"])["总分"].agg(
        平均分=("mean"),
        评委人数=("count"),
        最高分=("max"),
        最低分=("min")
    ...
    # 导出功能
    csv = valid_scores.to_csv(index=False)
